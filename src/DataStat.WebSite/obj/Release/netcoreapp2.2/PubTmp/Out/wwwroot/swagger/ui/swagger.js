...
$(function () {
    ...
    //设置标杆
    var _line = 0;//parseInt($(window).height() / 3);
    $(window).scroll(function () {
        //滚动730px，左侧导航固定定位
        if ($(window).scrollTop() > 10) {
            $('.leftMenu').css({ 'position': 'fixed', 'top': 10 })
        } else {
        ...
        //        $('.leftMenu li').eq(_i).addClass('active');
        //    }
        //    //如果到达页面底部，给左侧导航最后一个加active
        //    else if ($(document).height() == $(window).scrollTop() + $(window).height()) {
        //        $('.leftMenu li').removeClass('active');
        //        $('.leftMenu li').eq($('#resources > li').length - 1).addClass('active');
        //    }
        //});
    });
    $('.leftMenu li').click(function () {
        ...
        var _i = $(this).index();
        $('body, html').animate({ scrollTop: $('#resources > li').eq(_i).offset().top - _line }, 500);
    });
});